package io.vertx.shiva.signavio;

import java.util.Base64;
import java.util.List;
import java.util.LinkedList;
import java.io.ByteArrayInputStream;
import javax.ws.rs.core.Response;

import io.vertx.core.json.Json;

import org.apache.cxf.jaxrs.client.WebClient;
import org.apache.cxf.jaxrs.ext.multipart.Attachment;
import org.apache.cxf.jaxrs.ext.multipart.AttachmentBuilder;
import org.apache.cxf.jaxrs.ext.multipart.ContentDisposition;
import org.apache.cxf.jaxrs.ext.multipart.MultipartBody;

public class WebClientHelper{

  
    public WebClientHelper() {
    }

   

    public static String uploadToSignavioTest(String token)
    {
        String encodedString = "";
        byte[] imageByte = Base64.getDecoder().decode(encodedString);
        WebClient client =  WebClient.create("http://localhost:8080/api/v1/alliancebankofmalaysia/files");
        client.type("multipart/form-data");
        Attachment fileAttachment = new Attachment("file",  new ByteArrayInputStream(imageByte), new ContentDisposition("form-data; name=\"files\"; filename=\"signavio ldap.PNG\""));
        Attachment authAttachment = new AttachmentBuilder().id("Authorization").object(token).contentDisposition(new ContentDisposition("form-data; name=\"Authorization\"")).build();
        List<Attachment> atts = new LinkedList<Attachment>();
        atts.add(authAttachment);
        atts.add(fileAttachment);
        // atts.add(new Attachment("files", "application/octet-stream", new ByteArrayInputStream(imageByte)));
        // atts.add(new Attachment("Authorization","form-data","sEu4qiFVJqKNmNDsGIW1yKFFmZWH6otRqBtK"));
        MultipartBody body = new MultipartBody(atts);
        Response response= client.post(body);
       
        return response.readEntity(String.class);
    }

    public static String uploadToSignavio(byte[] fileByte, String fileName, String token )
    {
        WebClient client =  WebClient.create(new SignavioApiPathHelper().getFile());
        client.type("multipart/form-data");
        Attachment fileAttachment = new Attachment("file",  new ByteArrayInputStream(fileByte), new ContentDisposition("form-data; name=\"files\"; filename=\"" + fileName + "\""));
        Attachment authAttachment = new AttachmentBuilder().id("Authorization").object(token).contentDisposition(new ContentDisposition("form-data; name=\"Authorization\"")).build();
        List<Attachment> atts = new LinkedList<Attachment>();
        atts.add(authAttachment);
        atts.add(fileAttachment);
        // atts.add(new Attachment("files", "application/octet-stream", new ByteArrayInputStream(imageByte)));
        // atts.add(new Attachment("Authorization","form-data","sEu4qiFVJqKNmNDsGIW1yKFFmZWH6otRqBtK"));
        MultipartBody body = new MultipartBody(atts);
        Response response= client.post(body);
       
        return response.readEntity(String.class);
    }

    

    public static String postJson(String url, String authToken, Object pojo)
    {
        WebClient client =  WebClient.create(url);
        client.header("Authorization", authToken);
        client.type("application/json");
        return client.post(Json.encodePrettily(pojo), String.class);
       
    }
    public static String putJson(String url, String authToken, Object pojo)
    {
        WebClient client =  WebClient.create(url);
        client.header("Authorization", authToken);
        client.type("application/json");
        return client.put(Json.encodePrettily(pojo), String.class);
       
    }
}
